...
Things to add:
0)Main menu, Gameover, Victory screens
0.1)Enemies
1)Talking cutscene with penguin in level2
2)Shooting animation (fox's hand move forward)
3)Boss appear animation
4)Enemy AI
...
void Update3(GameState* game, float elapsed) {
	...
	worldToTileCoordinates(game->Player.position.x, playerBottom, &gridX, &gridY);//this is the center bottom of the player, but how do we check other tiles of player bottom??? question. Use for loop: -player.size.x/2 to player.size.x/2 in tile coords. nTiles = player.size.x/tileSize;
	//note the return value of gridX,gridY corresponds to which tile (defined in row=gridX,col=gridY) is in touch with the player's bottom.
	if (gridX < map3.mapWidth && gridX >= 0 && gridY < map3.mapHeight && gridY >= 0) {//if the player is still within Level bounds. This check is needed b/c otherwise map.mapData[y][x] would be accessing outside of it's array
		for (int i = 0; i < SolidTile.size(); i++) {
			if (map3.mapData[gridY][gridX] == SolidTile[i]) {//COLLISION on the bottom of player(if the playerBottom's grid position corresponds to the solid tile). Resolve it in terms of world coords
				float tileLeft = (float)gridX * tileSize;
				float tileRight = ((float)gridX + 1.0f) * tileSize;
				float tileTop = (float)(-gridY) * tileSize;
				float tileBottom = ((float)(-gridY) - 1.0f) * tileSize;
	...
	//collision on the top of player
	worldToTileCoordinates(game->Player.position.x, playerTop, &gridX, &gridY);
	if (gridX < map3.mapWidth && gridX >= 0 && gridY < map3.mapHeight && gridY >= 0) {//if player's top side is still in level bounds
		for (int i = 0; i < SolidTile.size(); i++) {
			if (map3.mapData[gridY][gridX] == SolidTile[i]) {//COLLISION on the top of player
				float tileLeft = (float)gridX * tileSize;
				float tileRight = ((float)gridX + 1.0f) * tileSize;
				float tileTop = (float)(-gridY) * tileSize;
				float tileBottom = ((float)(-gridY) - 1.0f) * tileSize;
	...
	//collision on the right side of player
	worldToTileCoordinates(playerRight, game->Player.position.y, &gridX, &gridY);
	if (gridX < map3.mapWidth && gridX >= 0 && gridY < map3.mapHeight && gridY >= 0) {//if player's right side is still in level bounds
		for (int i = 0; i < SolidTile.size(); i++) {
			if (map3.mapData[gridY][gridX] == SolidTile[i]) {//COLLISION on the right of player
				float tileLeft = (float)gridX * tileSize;
				float tileRight = ((float)gridX + 1.0f) * tileSize;
				float tileTop = (float)(-gridY) * tileSize;
				float tileBottom = ((float)(-gridY) - 1.0f) * tileSize;
	...
	//collision on the left side of player
	worldToTileCoordinates(playerLeft, game->Player.position.y, &gridX, &gridY);
	if (gridX < map3.mapWidth && gridX >= 0 && gridY < map3.mapHeight && gridY >= 0) {//if player's left side is still in level bounds
		for (int i = 0; i < SolidTile.size(); i++) {
			if (map3.mapData[gridY][gridX] == SolidTile[i]) {//COLLISION on the left of player
				float tileLeft = (float)gridX * tileSize;
				float tileRight = ((float)gridX + 1.0f) * tileSize;
				float tileTop = (float)(-gridY) * tileSize;
				float tileBottom = ((float)(-gridY) - 1.0f) * tileSize;
