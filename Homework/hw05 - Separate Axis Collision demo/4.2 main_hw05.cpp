...
3)resolving collision with rectangle -- something wrong with it
...
void Update(GameState* game, float elapsed, Entity player) {
	...
	for (int i = 0; i < game->Object.size(); i++) {//object 1
		for (int j = 0; j < game->Object.size(); j++) {//object 2
	...
	//for (int i = 0; i < game->Object.size(); i++) {
	//	//if the object revolves around player then adjust the position of the object based on a revolved matrix
	//	if (game->Object[i].relativeToPlayer == true) {
	//		for (int p = 0; p < game->Object[i].EntityVertices.size(); p++) {
	//			Vector3 ret = game->Object[i].modelMatrixMember * game->Object[i].EntityVertices[p];
	//			(game->Object[i].position.x, game->Object[i].position.y, game->Object[i].position.z) = (ret.x,ret.y,ret.z);
	//		}
	//	}
	//}
