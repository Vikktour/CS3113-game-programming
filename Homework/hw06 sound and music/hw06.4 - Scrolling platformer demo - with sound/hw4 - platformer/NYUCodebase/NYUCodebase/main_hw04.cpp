...
Need to add item collect sound
...
Mix_Chunk *flySound;
Mix_Chunk *winSound;
Mix_Chunk *waterCollect;
Mix_Chunk *bookReading;
...
void Update(GameState* game, float elapsed) {
	...
	for (int i = 0; i < game->WaterVec.size(); i++) {//speedboost
		...
		if (!(playerLeft > waterRight || playerRight < waterLeft || playerTop<waterBottom || playerBottom>waterTop)) {
			...
			Mix_PlayChannel(-1, waterCollect, 0);
		}
	}
	...
	for (int i = 0; i < game->BoostVec.size(); i++) {//speedboost
		...
		if (!(playerLeft > boostRight || playerRight < boostLeft || playerTop<boostBottom || playerBottom>boostTop)) {
			...
			Mix_PlayChannel(-1, bookReading, 0);
		}
	}
	...
	for (int i = 0; i < game->BoostVec.size(); i++) {//speedboost
		...
		if (!(playerLeft > princessRight || playerRight < princessLeft || playerTop<princessBottom || playerBottom>princessTop)) {
			if (playerTop >= game->DuckPrincess.position.y) {//an extra check to avoid accidental touch (from jumping from below)
				if (game->married == false) {//this check is so the winSound only perform once
...
void ProcessInput(GameState* game, float elapsed) {//make a copy of game so that it only applies acceleration for that instance of pressing
	if (keys[SDL_SCANCODE_LEFT]) {
		...
		if (game->Player.collidedBottom == true) {
			...
			if (game->Player.fly == false) {
				Mix_PlayChannel(-1, jumpSound, 0);
			}
		}
		if ((game->Player.fly == true) && (game->Player.collidedBottom == false)) {//if player can fly and is not on the ground.
			game->Player.velocity.y = jumpSpeed;
			if (playSoundTimer >= 1.0f) {
...
int main(int argc, char *argv[])
{
	...
	SDL_GL_MakeCurrent(displayWindow, context);
		...
		backgroundMusic = Mix_LoadMUS("BW-Bike-remix.mp3");//can be mp3
		Mix_PlayMusic(backgroundMusic, -1);


		jumpSound = Mix_LoadWAV("bounce-jump.wav"); Mix_VolumeChunk(jumpSound, 40); // set sound volume (from 0 to 128)
		flySound = Mix_LoadWAV("jumpfly.wav");
		winSound = Mix_LoadWAV("win-star.wav");
		waterCollect = Mix_LoadWAV("pickupItem.wav");
		bookReading = Mix_LoadWAV("bookFizz.wav");
		//Mix_PlayChannel(-1, jumpSound, -1);
	...
	*/
	...
	Mix_FreeChunk(waterCollect);
	Mix_FreeChunk(bookReading);
