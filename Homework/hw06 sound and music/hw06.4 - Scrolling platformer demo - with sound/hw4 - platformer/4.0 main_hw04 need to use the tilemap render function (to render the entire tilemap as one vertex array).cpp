...
void PlaceEntity(std::string type, float x, float y, GameState* game, FlareMap* map) {//this is for initalizing positions of entities
	...
	if (type == "DuckPlayer") {
			...
			//player.position.y = 20.0f * 16.0f;//comeback4
...
void InitializeGame(GameState *game) {//set the positions of all the entities
	...
	game->LevelMap[20][35].sprite.index = map.mapData[20][35];//comeback3 debug, tile not printing on screen
	game->LevelMap[20][35].position.x = game->Player.position.x+10.0f;// * 16.0f;
	game->LevelMap[20][35].position.y = game->Player.position.y+0.0f;// * 16.0f;
...
void Render(GameState* game, ShaderProgram* program) {//display the entities on screen
	...
	//	}
	//}
	game->LevelMap[2][20].Render(program);
