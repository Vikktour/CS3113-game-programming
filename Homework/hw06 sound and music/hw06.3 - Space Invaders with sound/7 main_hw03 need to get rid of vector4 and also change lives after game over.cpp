...
6)Initialize game function?
7)Lives go to 0 and gameover to mainmenu, lives not resetting?
*/
...
void checkEnemyWin() {//if enemy wins in gamelevel then switch to gameover
	...
	for (int i = 0; i < enemyCount; i++) {
		...
		//	mode = STATE_GAME_OVER;
		//}
		if (game.enemies[i].position.y < bottomScreen) {//comebackreturn //unable to switch back modes????
			mode = STATE_GAME_OVER;
		}
	}
}
