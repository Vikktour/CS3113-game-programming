...
float playerSpeed = 1.5f; float playerBulletSpeed = 5.0f; //global variable so it's easier to adjust
...
void UpdateGame(GameState &game, float elapsed) {
	...
	while (tempCount > 0) {
		...
		//float yRowGap = 0.5f;
		float aspect = game.enemies[0].sprite.width / game.enemies[0].sprite.height;
		float halfEnemySprite = aspect* game.enemies[0].sprite.size / 2.0f;
		...
		aspect = game.player.sprite.width / game.player.sprite.height;//NOTE that aspect*size is the widthvertex coordinates for the width
		float playerLeft = game.player.position.x - aspect * game.player.sprite.size / 2.0f;//divide by 2.0f b/c the position is at the center and the entity's left is half a width away
		float playerRight = game.player.position.x + aspect * game.player.sprite.size / 2.0f;
		if (playerLeft < leftScreen) {//check for player collision with wall
			game.player.position.x = leftScreen + aspect * game.player.sprite.size / 2.0f;
		}
		if (playerRight > rightScreen) {
			game.player.position.x = rightScreen - aspect * game.player.sprite.size / 2.0f;
		}
