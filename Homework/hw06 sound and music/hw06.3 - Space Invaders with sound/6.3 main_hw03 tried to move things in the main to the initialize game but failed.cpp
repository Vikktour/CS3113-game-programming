...
void ProcessGameInput(GameState &game,float elapsed) {
	...
	if (keys[SDL_SCANCODE_P]) {
		//pause the game (i.e. keep rendering but don't update() the positions
	}
...
class MainMenu {
public:
	MainMenu(GameLevel Level) :Level(Level) {}
	GLuint fontTexture;
	GameLevel Level;
...
void Render(MainMenu &mainMenu, GameLevel &Level, GameOver &Over) {//mode rendering (changes game state and draw the entities for that particular state)
	switch (mode) {
...
void Update(float elapsed, MainMenu &mainMenu, GameLevel &Level, GameOver &Over) {
	switch (mode) {
	case STATE_MAIN_MENU:
...
void ProcessInput(float elapsed, MainMenu &mainMenu, GameLevel &Level, GameOver &Over) {
	switch (mode) {
	case STATE_MAIN_MENU:
...
int main(int argc, char *argv[])
{
	...
	while (!done) {
		...
		Update(elapsed,mainMenu,Level,Over);
		Render(mainMenu,Level,Over);
		ProcessInput(elapsed,mainMenu,Level,Over);
