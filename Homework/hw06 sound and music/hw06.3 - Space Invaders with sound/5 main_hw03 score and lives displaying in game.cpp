...
float playerSpeed = 1.5f; float playerBulletSpeed = 5.0f; float enemyBulletSpeed = 0.1f; //global variable so it's easier to adjust
void DrawText(ShaderProgram *program, int fontTexture, std::string text, float size, float spacing);
int enemyCount = 21; int enemiesPerRow = 5; float enemySpeed = 0.3f; float yRowGapRatio = 1.5f;
...
//I didn't make a function for player shoot, but it's in Update(gamestate)
...
void RenderGame(const GameState &game) {
	...
	for (int i = 0; i < MAX_BULLETS; i++) {//draw enemy bullets --comeback, comment out for player to return
		bulletsModelMatrix[i].Identity();
...
void UpdateGame(GameState &game, float elapsed) {
	...
	while (tempCount > 0) {
		...
		enemyShoot(game, elapsed);//-comeback
	}
...
void ProcessGameInput(GameState &game,float elapsed) {
	...
	//game.bullets[1].position.x = game.player.position.x;//debug, need to somehow prevent hero from spray shooting
