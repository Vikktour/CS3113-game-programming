...
/* Sound begin */
int Mix_OpenAudio(int frequency, Uint16 format, int channels, int chunksize);//initializes SDL_mixer with frequency,format,channel,buffersize
...
void enemyShoot(GameState &game, float elapsed) {
	...
	//while ((game.enemies[shooter].position.x > rightScreen) || (game.enemies[shooter].position.x < leftScreen) || (game.enemies[shooter].position.y > topScreen)
	//	|| (game.enemies[shooter].position.y < bottomScreen)) {//if shooter is not within view then pick another shooter
	//	if (game.score < enemyCount) {//if there are still enemies left on screen
	//		int shooter = rand() % enemyCount;
	//	}
	//}
	while (game.enemies[shooter].position.y < bottomScreen) {//if shooter is not within view then pick another shooter
		if (game.score < enemyCount) {//if there are still enemies left on screen
			shooter = rand()%enemyCount;// % enemyCount;
		}
	}
...
void ProcessGameInput(GameState &game,float elapsed, Mix_Chunk* playerShootSound) {
	...
	if (keys[SDL_SCANCODE_UP]) {
		...
		if (game.playerNoShoot >= game.playerReloadTime) {
			...
			Mix_PlayChannel(-1, playerShootSound, 0);//play shooting noise
		}
	}
...
public:
	...
	void ProcessInput(Mix_Music* backgroundMusic) {//SDL_SCANCODE to switch states (e.g. menu to level)
...
public:
	...
	void ProcessInput(float elapsed, Mix_Chunk* playerShootSound) {
		ProcessGameInput(*game, elapsed, playerShootSound);
	}
};
...
void ProcessInput(float elapsed, Mix_Chunk* playerShootSound, Mix_Music* backgroundMusic) {
	switch (mode) {
	case STATE_MAIN_MENU:
		mainMenu.ProcessInput(backgroundMusic);
		break;
	case STATE_GAME_LEVEL:
		Level.ProcessInput(elapsed, playerShootSound);
		break;
	case STATE_GAME_OVER:
		Over.ProcessInput();
		break;
...
int main(int argc, char *argv[])
{
	...
	/* SOUND BEGIN */
	//SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO);//flags for..?
	Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 4096);
	
	//Soundeffect
	Mix_Chunk* playerShootSound;
	playerShootSound = Mix_LoadWAV("player-shoot.wav");
	//Mix_VolumeChunk(playerShootSound, -1);//set sound volume (from 0 to 128)
	//Mix_PlayChannel(-1, playerShootSound, 2);//This players the sound. Input:channel,chunk,loops. channel=-1 means first play on free channel, loop=-1 means loop infinite times. Returns channel used to play sound.
	
	//Mix_Chunk* enemyShootSound;//make global variable so we don't have to pass into update()
	enemyShootSound = Mix_LoadWAV("enemy-shoot.wav");

	//music
	Mix_Music *visionMusic;
	visionMusic = Mix_LoadMUS("Second-Vision.mp3");//loading the sound
	//Mix_PlayMusic(visionMusic, -1);//music,#loops. -1 means loop forever.


	//Mix_HaltMusic();//pause music
	//Mix_VolumeMusic(30);//set music volume (from 0 to 128)


	//Mix_HaltChannel(3); // stop playback on channel 3 (-1 to stop all sound)
	...
	while (!done) {
		...
		Update(elapsed);
		Render();
		ProcessInput(elapsed,playerShootSound,visionMusic);

		SDL_GL_SwapWindow(displayWindow);
	}
	//upon quit, clean up the music and sounds. What's the point of this??
	Mix_FreeChunk(playerShootSound);
	Mix_FreeChunk(enemyShootSound);
	Mix_FreeMusic(visionMusic);
