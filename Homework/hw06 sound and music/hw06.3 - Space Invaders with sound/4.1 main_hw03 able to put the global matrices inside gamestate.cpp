...
float playerSpeed = 1.5f; //global variable so it's easier to adjust
...
public:
	...
	float reloadTime = 1.0f; //this is the time before the next bullet can be fired, so that the bullets aren't sprayed on top of each other
	float noShoot = reloadTime; int bulletIndex = 0;
...
GameState state;
void RenderGame(const GameState &game) {//--comeback and add const???????????doesn't work
...
void ProcessGameInput(GameState &game,float elapsed) {
	...
	//game.bullets[1].position.x = game.player.position.x;//debug --comeback.. need to somehow prevent hero from spray shooting
	//game.bullets[1].position.y = game.player.position.y + 1.0f;

	if (keys[SDL_SCANCODE_UP]) {
		//player shoots bullets upward
		if (game.noShoot >= game.reloadTime) {
			...
			game.bullets[game.bulletIndex].velocity.y = 5.0f;
			game.noShoot = 0.0f;
			game.bulletIndex++;
		}
		if (game.bulletIndex > MAX_BULLETS - 1) {
...
int main(int argc, char *argv[])
{
	...
	mainMenu.fontTexture = LoadTexture(RESOURCE_FOLDER"1font.png");
