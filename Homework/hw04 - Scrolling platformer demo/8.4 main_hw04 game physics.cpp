	...
	for (int y = 0; y < map.mapHeight; y++) {//comeback2
		for (int x = 0; x < map.mapWidth; x++) {
			...
			//if (map.mapData[y][x] == 121) {
				...
				//	if (penetrationY < tileSize / 2.0f) {//if the penetration is small then we know we're dealing with 2 blocks near each other.
				//		game->Player.position.y += 1.0f;//penetrationY + 0.001f;
				//	}
				//}

				if (fabs((game->Player.position.y - halfLength) - staticTop)<= halfLength) {//&& game->Player.position.x - halfLength >= staticLeft && game->Player.position.x + halfLength <= staticRight && game->Player.position.y + halfLength >= staticTop) {//collision: bottom of player hit top of static entity
					game->Player.collidedBottom = true;//what to do with this boolean????
					float penetrationY = fabs(staticTop - (game->Player.position.y - halfLength));
					game->Player.position.y += 1.0f;//penetrationY + 0.001f;
